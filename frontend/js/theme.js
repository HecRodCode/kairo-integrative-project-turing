...
const resources = {
  en: {
    translation: {
      ...
      eco: {
        title: 'Learn the tools used by the industry',
      },
      cta: {
  ...
  es: {
    translation: {
      ...
      eco: {
        title: 'Aprende las herramientas que usa la industria',
      },
      cta: {
...
function applyTranslations() {
  ...
  if (langLabel) langLabel.innerText = i18next.language.toUpperCase();
}

document.addEventListener('DOMContentLoaded', () => {
  const moonIcon = document.getElementById('moon-icon');
  const sunIcon = document.getElementById('sun-icon');
  updateThemeIcons(localStorage.getItem('theme') === 'dark', moonIcon, sunIcon);
  const langBtn = document.getElementById('language-toggle');
  if (langBtn) {
    langBtn.addEventListener('click', () => {
      const newLang = i18next.language === 'en' ? 'es' : 'en';
      i18next.changeLanguage(newLang, () => {
        localStorage.setItem('kairo-lang', newLang);
